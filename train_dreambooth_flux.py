...
def parse_args(input_args=None):
    ...
    parser.add_argument("--clip_l", type=str, default=None, required=False)
    parser.add_argument("--t5xxl", type=str, default=None, required=False)
    parser.add_argument("--vae", type=str, default=None, required=False)
    parser.add_argument(
        "--disable_mmap_load_safetensors", type=bool, default=False, required=False
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            item = dataset["train"][image_column][0]
            if isinstance(item, str) and validators.url(item):
                from datasets import Image as Img

                dataset = dataset.cast_column(image_column, Img())
...
def main(args):
    ...
    if args.clip_l:
        # text_encoder_cls_one = flux_utils.load_clip_l(args.clip_l, weight_dtype, "cpu", args.disable_mmap_load_safetensors)
        text_encoder_cls_one = import_model_class_from_model_name_or_path(
            args.clip_l, args.revision
        )
    else:
        text_encoder_cls_one = import_model_class_from_model_name_or_path(
    ...
    if args.t5xxl:
        # text_encoder_cls_two = flux_utils.load_t5xxl(args.t5xxl, weight_dtype, "cpu", args.disable_mmap_load_safetensors)
    ...
    if args.vae:
        # vae = flux_utils.load_ae(args.ae, weight_dtype, "cpu")
    ...
    if args.with_prior_preservation:
        if not args.train_text_encoder:
            class_prompt_hidden_states, class_pooled_prompt_embeds, class_text_ids = (
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        # batch prompts on all training steps
            ...
            tokens_two = tokenize_prompt(
                tokenizer_two, args.instance_prompt, max_sequence_length=512
            )
            if args.with_prior_preservation:
                ...
                class_tokens_two = tokenize_prompt(
                    tokenizer_two, args.class_prompt, max_sequence_length=512
                )
            ...
            if (
                ...
                pipeline = FluxPipeline.from_pretrained(
                    ...
                    text_encoder=accelerator.unwrap_model(
                        text_encoder_one, keep_fp32_wrapper=False
                    ),
                    text_encoder_2=accelerator.unwrap_model(
                        text_encoder_two, keep_fp32_wrapper=False
                    ),
